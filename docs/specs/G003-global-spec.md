...
- Persist signed-in users' saved lists through Supabase accounts while still allowing temporary guest lists before sign-up or sign-in.
- Provide an admin-only catalog management path for adding, editing, and deleting runtime catalog items after account-backed persistence exists.
...
- Admin archive/unarchive behavior.
- Separate admin login systems.

## MVP Scope

- Email/password account sign-up and sign-in with required display names.
- Supabase-backed saved lists for signed-in accounts.
- Browser-local guest lists that migrate automatically after sign-up or sign-in.
- Homepage `My Lists` section that mirrors the first four cards from the To-do page order, including generated default preset-derived lists for new accounts and guest profiles.
...
- Runtime admin-created catalog records and admin-uploaded catalog images.
...
- A signed-in player can return on another device and load account-backed saved lists.
- A guest player can start locally and later migrate temporary lists into a signed-in account.
...
- `technical/T002-local-storage-and-data-architecture.md` for Supabase persistence, guest migration, and account roles.
