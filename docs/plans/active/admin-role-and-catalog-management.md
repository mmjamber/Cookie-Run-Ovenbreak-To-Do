...
Turn the admin-account idea into durable specs for a later implementation pass. Admins should be able to sign in, be recognized as admin-owned accounts, and manage catalog items for cookies, pets, and treasures without exposing those tools to ordinary users.
...
- The Supabase spec migration plan already reserves future account roles or permissions.
- Admin recognition should use a durable role or permission field linked to an authenticated account.
...
- Admin-only pages, actions, and storage writes must be protected by server-side or database-level authorization, not only hidden buttons.
...
- `docs/specs/G004-functional-map.md` - add admin login, admin dashboard, and catalog item creation flows.
...
- `docs/specs/user-interface/UI002-page-shell-and-navigation.md` - define where admin entry points appear for signed-in admin accounts.
...
- Admins must log in before they can access admin options.
- An admin account must be identified by a role, permission, or equivalent trusted account property.
...
- Confirmation of the generated catalog id, release/order number, storage object key, and public display URL before publishing.
...
- Display-ready derivatives may be generated for catalog rendering, but they must keep a reference back to the original upload.
...
- Draft, rejected, archived, or needs-review admin uploads should not appear in public catalogs.
- Archive or unpublish should be the normal removal behavior so existing saved-list references are not broken.
...
- Unpublish or archive an item without deleting saved-list references.
- Review catalog records marked `needsReview`.
- Approve or reject pending uploaded catalog items before they become public.
...
- Mark items as hidden from catalog filters temporarily without removing them from storage.
...
- User/account management beyond assigning or removing admin role.
- Full analytics dashboards.
- Automatic fetching from official game sources or APIs.

## Spec Tasks

- [ ] Update the account/auth specs with an admin role or permission model after the Supabase account plan is approved.
...
- [ ] Update the data model spec with runtime catalog records, upload records, approved, draft, rejected, archived, and needs-review catalog item states.
...
- [ ] Add user stories for admin login, adding a cookie, adding a pet, adding a treasure, reviewing a pending item, editing catalog metadata, and replacing an item image.
...
- [ ] Implement admin role checks using the approved account system.
...
- [ ] Implement image validation, runtime object-storage upload, display derivative generation, and naming collision checks.
...
- Should uploaded images be converted to `.webp`, or should the app preserve the original extension when it is already web-safe?
- Who is allowed to grant or revoke admin role: database owner only, existing admins, or a hard-coded development allowlist?
- Should release/order numbers be manually entered, auto-incremented within rarity/type, or derived from filename order?
- Should archived admin-created items remain visible inside existing saved lists with an archived marker, or render like ordinary historical catalog items?
- Which object-storage provider and bucket policy should runtime admin uploads use?
